...
class Doctor:  # creates doctor object, class #1
    def __init__(self, doctor_id="", name="", specialization="", working_time="", qualification="", room_number=""):  # doctor properties
...
class DoctorManager:  # used by display_doctor_menu for managing doctor, class #2

    def __init__(self):
        self.doctors = []
        self.read_doctors_file()

    def format_dr_info(self, doctor):  # formats input into the usable format for the text file
    ...
    def enter_dr_info(self):  # enter in data for a new doctor
    ...
    def read_doctors_file(self):  # used with Doctor to create objects from the file
    ...
    def display_doctor_info(self, doctor):  # displays individual doctor's profile
    ...
    def edit_doctor_info(self):  # edits doctors profile, then adds back to file
    ...
    def display_doctors_list(self):  # shows the list of doctors in the file
        print("Doctor ID\tName\tSpecialization\tWorking Time\tQualification\tRoom Number")
        for doctor in self.doctors:
            self.display_doctor_info(doctor)
            print(" ")

    def write_list_of_doctors_to_file(self):  # writes the list of doctors to the file
        with open("doctors.txt", "w") as f:
            for doctor in self.doctors:
                f.write(self.format_dr_info(doctor))

    def add_dr_to_file(self):  # adds doctor to the file
...
class Patient:  # creates patient object, class #3
    def __init__(self, pid="", name="", disease="", gender="", age=""):  # properties of patient objects
        self.pid = pid
        self.name = name
...
class PatientManager:  # used by display_patient_menu for managing patient, class #4
    def __init__(self):
        self.patients = []
        self.read_patients_file()

    def format_patient_info_for_file(self, patient):  # formats input into usable code for the text file
        return f"{patient.pid}_{patient.name}_{patient.disease}_{patient.gender}_{patient.age}\n"

    def enter_patient_info(self):  # used with Patient, gathers information for the object
        pid = input("\nEnter patient ID: ")
    ...
    def read_patients_file(self):  # opens patients.txt, skips first line and reads the rest of the data
    ...
    def display_patient_info(self, patient):  # displays individual patient's information
        print(f"\n{patient.pid}\t", end="")
    ...
    def edit_patient_info_by_id(self):  # search for patient's ID, then edit their profile
    ...
    def display_patients_list(self):  # displays list of patients
        print("Patient ID\tName\tDisease\tGender\tAge")
        for patient in self.patients:
            self.display_patient_info(patient)
            print(" ")

    def write_list_of_patients_to_file(self):  # writes the list of patients to the file
        with open("patients.txt", "w") as f:
            for patient in self.patients:
                f.write(self.format_patient_info_for_file(patient))

    def add_patient_to_file(self):  # adds patient to the file
...
class Management:  # inital menu, then able to select either doctors/patients, class #5
    def __init__(self):
        self.doctor_manager = DoctorManager()
        self.patient_manager = PatientManager()

    def display_menu(self):  # Main Menu for program
        while True:
            choice1 = input("\nWelcome to Alberta Hospital (AH) Management System\nSelect from the following options, or "
                            "select 3 to stop:\n1 -  Doctors\n2 -  Patients\n3 -  Exit Program\n")
            if choice1 == "1":  # opens doctor menu
                self.display_doctor_menu()
            elif choice1 == "2":  # opens patient menu
                self.display_patient_menu()
            elif choice1 == "3":  # ends program
                print("\nThanks for using the program! Bye")
                break
            else:
                print("Invalid choice. Please try again.")

    def display_doctor_menu(self):  # displays doctor menu
        while True:
    ...
    def display_patient_menu(self):  # displays patient menu
        while True:
...
run = Management()  # creates object from Management
run.display_menu()  # runs display_menu
